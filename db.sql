...
SELECT u.id, '127.0.0.1', 'Seeder/1.0', DATE_SUB(NOW(), INTERVAL 2 DAY), 'valid'
FROM users u
WHERE NOT EXISTS (SELECT 1 FROM login_logs)
LIMIT 1;

INSERT INTO login_logs (user_id, ip_address, browser_agent, login_time, status)
SELECT u.id, '127.0.0.1', 'Seeder/1.0', DATE_SUB(NOW(), INTERVAL 1 DAY), 'blocked'
FROM users u
WHERE NOT EXISTS (SELECT 1 FROM login_logs WHERE status = 'blocked')
LIMIT 1;

INSERT INTO login_logs (user_id, ip_address, browser_agent, login_time, status)
SELECT u.id, '127.0.0.1', 'Seeder/1.0', DATE_SUB(NOW(), INTERVAL 12 HOUR), 'verification'
FROM users u
